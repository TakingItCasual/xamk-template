% Use "lessxamk" to disable most of the official XAMK formatting.
% Use "online" to stop minted from using out/ as an output directory (use on Overleaf).
% Use "minted" to enable the creation of code blocks with syntax highlighting.
% Use "listings" to provide an environment to display monospaced commands/paths/etc.
% Use "pgfplots" to load the pgfplots package (used for graphs) and set some default styling.
% Use "includebib" to enable usage of biblatex/biber (.bib file(s) must be imported).
\documentclass[online, minted, listings, pgfplots, includebib]{xamk} % Use the xamk.cls document class
...
\begin{document}
	...
	% Use the lof command option to show the list of figures (relevant references removed from bib)
	% Use the lot command option to show the list of tables (relevant references removed from bib)
	% Use the notbib command option to hide the bibliography (e.g. if all references are in lof)
    \makebibliography[lof, lot] % Automatically generates the bibliography using biblatex

	% Appendix sections should be within the appendices environment
	\begin{appendices}
        \include{appendix1}
    \end{appendices}
\end{document}
