...
% Prints the bibliography, and (if configured) list of figures/tables. Use the lof (and/or lot) option(s) to print the list of figures/tables and move figure/table bibliography entries there.
\end{code}
%
\begin{code}[firstnumber=last]{latex}[title={Caption commands},label=code:caption-cmds]
\labelcaption{<label>}{<caption>}
% Defined for the figure and table environments. Creates label (prefixed with "fig:" for figures and "tab:" for tables). Must be used instead of the \caption and \label commands in these environments.
\citecaption{<cite key/label>}{<caption>}
% Same as the \labelcaption command, but also inserts citation at end of caption, and configures the bibliography entry to appear under the list of figures/tables rather than the bibliography, if appropriate options set for \makebibliography.
\end{code}
%
