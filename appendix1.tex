...
% Environment. Provided with minted class option. Minted options (starting line number, font size, etc.) can be specified before the language, while TCBListing options (title, color, etc.) can be specified after. Supported languages can be seen with the command "pygmentize -L lexers", or from https://pygments.org/docs/lexers/.
\codeinput{<file>}[<minted options>]{<language>}[<tcblisting options>]
% Same as code environment, but gets contents from file. (The firstline and lastline minted options may be of use.)
{cmd}[<tcblisting options>]
% Environment. Provided with listings class option. TCBListing options (title, line prefix, etc.) can optionally be specified.
\cmdinput{<file>}[<tcblisting options>]
% Same as cmd environment, but gets contents from file.
\end{code}
